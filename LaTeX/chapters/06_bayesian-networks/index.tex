...
Probabilistic graphical models provide a convenient framework to represent and manipulate complex probability distributions. Bayesian Networks (BNs) are a formalism to represent probabilistic relationships among a set of random variables. They allow one to:
\begin{itemize}
    ...
    (e.g., discrete versus continuous variables).
\end{itemize}
...
\begin{itemize}
    ...
    \item Each directed edge represents a direct dependency between the connected variables;
\end{itemize}
It is important to note that the absence of a direct edge between two nodes does not imply that the variables are unconditionally independent.

The given structure $\mathcal{G}$ of a BN encodes these independence assumptions:
\begin{equation}
    \label{eq:localIndependencies}
    \mathcal{I}_l(\mathcal{G}) = \{ \forall i\; \boldsymbol{x}_i \perp \text{NonDescendants}_{\boldsymbol{x}_i} \mid \text{Parents}_{\boldsymbol{x}_i} \}
\end{equation}
Each variable $\boldsymbol{x}_i$ is independent of its non-descendants given its parents. 

\begin{figure}[H]
\centering
    \subimport{tikz/}{example-bn}
...
\[ p(\boldsymbol{x}_1, \boldsymbol{x}_2, \ldots, \boldsymbol{x}_m) = \prod_{i=1}^{m} p(\boldsymbol{x}_i \mid \text{Parents}_{\boldsymbol{x}_i}) \]
...
\begin{itemize}
    ...
    \[ \{\forall i\; \boldsymbol{x}_i \perp \text{NonDescendants}_{\boldsymbol{x}_i} \mid \text{Parents}_{\boldsymbol{x}_i} \}\]
    ...
    \[ p(x_1, x_2, \ldots, x_m) = \prod_{i=1}^{m} p(x_i \mid x_1, x_2, \ldots, x_{i-1}) \]
    \item Since we ordered the variables in a topological order, now $\text{Parents}_{x_i} \subseteq \{x_1, x_2, \ldots, x_{i-1}\}$.
    We derive that:
    \[ p(x_i \mid x_1, x_2, \ldots, x_{i-1}) = p(x_i \mid \text{Parents}_{x_i}) \]
\end{itemize}
Combining the last two equations, we obtain the factorization property:
\[ p(x_1, x_2, \ldots, x_m) = \prod_{i=1}^{m} p(x_i \mid \text{Parents}_{x_i}) \]
...
\begin{itemize}
    ...
    \item Consider now the variable $x_m$. By the product rule ($p(X, Y) = p(Y\mid X) p(X)$), we can express the conditional distribution as:
...
\end{itemize}
We can subsequently simplify the equation, since $\sum_{x_m} p(x_m| \text{Parents}_{x_m}) = 1$, obtaining:
\[
...
\begin{figure}[H]
\centering
    \subimport{tikz/}{example-bn-structure}
...
Before introducing the concept of D-separation, we need to better understand independence and conditional independence.
...
\definition{Conditional independence}{
    We say that two variables $a, b$ are conditionally independent given a third variable $c$ (written $a \perp b \mid c$) if:
    \[ P(a, b \mid c) = P(a \mid c) P(b \mid c) \]
}
\end{figure}
Both independence and conditional independence can be verified by applying basic rules of probability.
Instead, BNs allow one to verify these properties through the concept of D-separation.
...
\begin{figure}[H]
\centering
    \subimport{tikz/}{tail-to-tail-connection}
...
\begin{itemize}
    ...
    \item \textbf{Conditionally independent}: we need to check that $P(a, b \mid c) = P(a \mid c) P(b \mid c)$;
    We can apply the product rule:
    \[ P(a, b \mid c) = \frac{P(a, b, c)}{P(c)} \]
    We can then substitute the joint distribution from eq \ref{eq:tailToTail}:
    \[ P(a, b \mid c) = \frac{P(a \mid c) P(b \mid c) P(c)}{P(c)} = P(a \mid c) P(b \mid c) \]
...
In a head-to-tail connection, variable $a$ is a parent of variable $c$, which is a parent of variable $b$.
It is called head-to-tail because $c$ has one incoming edge (head) from $a$ and one outgoing edge (tail) to $b$.
\begin{figure}[H]
\centering
    \subimport{tikz/}{head-to-tail-connection}
    \caption{Head-to-tail connection.}
\end{figure}
The joint distribution can be expressed as:
\[ p(a,b,c) = p(b \mid c) p(c \mid a) p(a) \]
Note that, using Bayes' theorem, we can rewrite $p(c \mid a) p(a)$ as $\frac{p(a \mid c) p(c)}{p(a)}$, obtaining:
\[ p(a,b,c) = p(b \mid c) p(a \mid c) p(c) \]
This is exactly the same form as the tail-to-tail case \ref{eq:tailToTail}.
For completeness, we will repeat the independency analysis for clarity.
\begin{itemize}
    \item \textbf{Independent}: we need to check that $P(a, b) = P(a) P(b)$;
    \[ P(a, b) = \sum_{c} P(a, b, c) = \sum_{c} p(b \mid c) p(c \mid a) p(a) \neq P(a) P(b)\]
    So $a$ and $b$ are \textbf{not independent}.
    \item \textbf{Conditionally independent}: we need to check that $P(a, b \mid c) = P(a \mid c) P(b \mid c)$;
    \[ P(a, b \mid c) = \frac{P(a, b, c)}{P(c)} = \frac{p(b \mid c) p(c \mid a) p(a)}{P(c)} = P(a \mid c) P(b \mid c) \]
    So $a$ and $b$ are \textbf{conditionally independent} given $c$.
\end{itemize}
Intuitively, if we already know the value of $c$, knowing $a$ provides no additional information about $b$, and vice versa.
Unlike the tail-to-tail case, here $c$ is an intermediary variable between $a$ and $b$.
\paragraph{Example:}
...
information about whether we get wet. For both cloudy and clear skies, if it is raining we will get wet.
...
\begin{figure}[H]
\centering
    \subimport{tikz/}{head-to-head-connection}
...
\begin{itemize}
    ...
    \[ P(a, b) = \sum_{c} P(a, b, c) = \sum_{c} p(c\mid a,b) p(a) p(b)\]
    ...
    \[ P(a, b \mid c) = \frac{P(a, b, c)}{P(c)} = \frac{p(c\mid a,b) p(a) p(b)}{P(c)} \neq P(a \mid c) P(b \mid c) \]
...
A \textit{descendant} of a node $x$ is any node that can be reached by following directed edges starting from $x$.
...
\begin{figure}[H]
    \centering
    \subimport{tikz/}{general-head-to-head}
...
For instance, in the previous example of burglary, earthquake, and alarm ringing, adding a new variable $d$ representing a call from the alarm company.
...
This rule is valid for any descendant of a head-to-head node, but the further down the descendant is in the graph, the weaker the dependency between the parents becomes.
\begin{figure}[H]
    \centering
    \resizebox{0.8\textwidth}{!}{
    \subimport{tikz/}{d-separation-summary}
    }
...
\definition{D-separation}{
    \begin{itemize}
        ...
        \[ \text{D-sep}(\boldsymbol{A}, \boldsymbol{B} \mid \boldsymbol{C}) \]
        \end{itemize}
}
\end{figure}
...
\begin{figure}[H]
\centering
    \subimport{tikz/}{example-bn-dsep}
...
\begin{enumerate}
    ...
    \begin{itemize}
        ...
        \item Node $E$ is in a head-to-head connection and is not observed. One of its children ($C$) is observed, meaning
...
\definition{Global independencies}{
    \[ \mathcal{I}(\mathcal{G}) = \{ (A \perp B \mid C): \text{D-sep}(A, B \mid C) \} \]
}
\end{figure}
...
\end{figure}
As we have previously seen, for a structure $\mathcal{G}$ to be an I-map for a distribution $p$ it is not necessary that $\mathcal{I}(\mathcal{G}) = \mathcal{I}(p)$ (does not need to encode all independencies in $p$).
As a consequence, a fully connected graph (no independencies) is an I-map for any distribution $p$ over the same set of variables. Such a graph is obviously not very useful, as it does not provide any information about independencies in $p$.

To overcome this limitation, we can define a \textbf{minimal I-map}.
\begin{figure}[H]
...
\end{figure}
This means it is impossible to remove any edge from $\mathcal{G}$ without introducing independencies that do not hold in $p$.
Note that a minimal I-map does not necessarily encode all independencies in $p$. 
...
Here is a list of practical guidelines to build a BN structure:
\begin{itemize}
    \item Get together with domain experts. This allows to gather prior knowledge about the relationships between variables;
    \item Define relevant variables for both observable entities and quantities you wish to predict. 
    Also, including latent variables can help to better represent the dependencies in the data;
    \item When adding edges, follow causal relationships when possible. 
    For example, if variable $A$ causes variable $B$, add a directed edge from $A$ to $B$;
    \item Avoid assigning zero probabilities to conditional probability distributions.
